...
private:
	...
	void runSimulation(std::unordered_map<std::string, double> &params, std::vector<result_ns::Result> &results);
	...
	bool handleAction(result_ns::Result &result, double &dailyCashReset, double &dailyCashNoReset, Action &action, Quote &quote, trade_ns::Trade &trade,
		bool &activePositions, std::unordered_map<std::string, double> &params);
	void sendResults(std::unordered_map<std::string, double> &params, std::vector<result_ns::Result> &results, std::mutex &m);
};
